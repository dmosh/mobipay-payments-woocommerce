...
/**
 * mobipay Mobile Payments Gateway.
 *
 * Provides a mobipay Mobile Payments Payment Gateway.
 *
 * @class       WC_Gateway_mobipay
 * @extends     WC_Payment_Gateway
 * @version     2.1.0
 * @package     WooCommerce/Classes/Payment
 */
class WC_Gateway_mobipay extends WC_Payment_Gateway {
	...
	protected function setup_properties() {
		$this->id                 = 'mobipay';
		$this->icon               = apply_filters( 'woocommerce_mobipay_icon', plugins_url('../assets/mainlogo.png', __FILE__ ) );
		$this->method_title       = __( 'MobiPay Mobile Payments', 'mobipay-payments-woo' );
		$this->api_key            = __( 'Add API Key', 'mobipay-payments-woo' );
		$this->widget_id          = __( 'Add Widget ID', 'mobipay-payments-woo' );
		$this->method_description = __( 'Have your customers pay with MobiPay Mobile Payments.', 'mobipay-payments-woo' );
	...
	public function init_form_fields() {
		$this->form_fields = array(
			...
			'api_key'             => array(
				'title'       => __( 'API Key', 'mobipay-payments-woo' ),
				'type'        => 'text',
				'description' => __( 'Add your API key', 'mobipay-payments-woo' ),
				'desc_tip'    => true,
			),
			'widget_id'           => array(
				'title'       => __( 'Widget ID', 'mobipay-payments-woo' ),
				'type'        => 'text',
				'description' => __( 'Add your Widget key', 'mobipay-payments-woo' ),
				'desc_tip'    => true,
			),
			'description'        => array(
	...
	private function is_accessing_settings() {
		if ( is_admin() ) {
			...
			if ( ! isset( $_REQUEST['section'] ) || 'mobipay' !== $_REQUEST['section'] ) {
				return false;
			}
	...
	private function load_shipping_method_options() {
		...
		foreach ( WC()->shipping()->load_shipping_methods() as $method ) {
			...
			$options[ $method->get_method_title() ][ $method->id ] = sprintf( __( 'Any &quot;%1$s&quot; method', 'mobipay-payments-woo' ), $method->get_method_title() );
			...
			foreach ( $zones as $zone ) {
				...
				foreach ( $shipping_method_instances as $shipping_method_instance_id => $shipping_method_instance ) {
					...
					$option_instance_title = sprintf( __( '%1$s (#%2$s)', 'mobipay-payments-woo' ), $shipping_method_instance->get_title(), $shipping_method_instance_id );

					// Translators: %1$s zone name, %2$s shipping method instance name.
					$option_title = sprintf( __( '%1$s &ndash; %2$s', 'mobipay-payments-woo' ), $zone->get_id() ? $zone->get_zone_name() : __( 'Other locations', 'mobipay-payments-woo' ), $option_instance_title );
	...
	public function process_payment( $order_id ) {
		...
		if ( $order->get_total() > 0 ) {
			$this->mobipay_payment_processing( $order );
		}
	}

	private function mobipay_payment_processing( $order ) {
	...
	/**
	 * Change payment complete order status to completed for mobipay orders.
	 *
	 * @since  3.1.0
	...
	public function change_payment_complete_order_status( $status, $order_id = 0, $order = false ) {
		if ( $order && 'mobipay' === $order->get_payment_method() ) {
			$status = 'completed';
		}
		return $status;
	}
